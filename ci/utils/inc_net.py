...
from convs.linears import SimpleContinualLinear, SimpleMHLinear
...
class FinetuneIncrementalMHNet(FinetuneIncrementalNet):
    ...
    def update_fc(self, nb_classes, freeze_old=True):
        if self.fc is None:
            self.fc = self.generate_fc(self.feature_dim, nb_classes, self.args)
        else:
            self.fc.update(nb_classes, freeze_old=freeze_old)
    ...
    def generate_fc(self, in_dim, out_dim, args):
        fc = SimpleMHLinear(in_dim, out_dim, args=args)
