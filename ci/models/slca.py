...
from utils.inc_net import FinetuneIncrementalNet, FinetuneIncrementalMHNet
...
class SLCA(BaseLearner):
    def __init__(self, args):
        ...
        if 'virtual_batch' in args.keys():
            global virtual_batch
            virtual_batch = args['virtual_batch']
            print('set virtual_batch to ', virtual_batch)
        if 'num_workers' in args.keys():
            global num_workers
            num_workers = args['num_workers']
            print('set num_workers to ', num_workers)
        if 'bcb_lrscale' in args.keys():
            self.bcb_lrscale = args['bcb_lrscale']
        else:
            self.bcb_lrscale = 1.0 / 100
        if self.bcb_lrscale == 0 or self.bcb_lrscale == 0.0:
            self.fix_bcb = True
        else:
            self.fix_bcb = False
        print('fix_bcb', self.fix_bcb)
    ...
    def incremental_train(self, data_manager):
        ...
        if self.save_before_ca:
            self.save_checkpoint(self.log_path + '/' + self.model_prefix + '_seed{}_before_ca'.format(self.seed),
                                 head_only=self.fix_bcb)
    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler):
        ...
        acc_point = virtual_batch / batch_size  # virtual_batch= 128,  batch_size=4
        for epoch in range(1, run_epochs + 1):
            self._network.train()
            optimizer.zero_grad()
            losses = 0.
            for i, (_, inputs, targets) in enumerate(train_loader):  # batch piccolo
                ...
                loss = loss / acc_point
                loss.backward()
                if (i + 1) % acc_point == 0 or i == len(train_loader) - 1:  # check i +1 = 32
                    optimizer.step()
                    optimizer.zero_grad()
                losses += loss.item()
            ...
            if epoch % 5 == 0:
                ...
                if 'wandb' in self.args.keys():
                    if self.args["wandb"]:
                        wandb.log(
                            {f"Task_{self._cur_task}/trainAcc": train_acc, f"Task_{self._cur_task}/testAcc": test_acc})
            else:
    ...
    def _stage2_compact_classifier(self, task_size):
        ...
        self._network.eval()
        if "mh" in self.args['model_name']:
            # need mh in training mode to be able to backward with correct sizes
...
class MHHeads(SLCA):
    def __init__(self, args):
        super().__init__(args)
        self._network = FinetuneIncrementalMHNet(args['convnet_type'], pretrained=True, args=args)
        self.freeze_keys_ca = args['freeze_keys_ca']
    ...
    def incremental_train(self, data_manager):
        ...
        self._compute_class_mean(data_manager, check_diff=False, oracle=False)
        if self._cur_task > 0 and ca_epochs > 0:
            print("Classifier Alignment...")
            if self.freeze_keys_ca:
                self._network.fc.heads[-1][0].beta_k = 0.0
                self._network.fc.heads[-1][0].scramble = False
            self._stage2_compact_classifier(task_size)
            if len(self._multiple_gpus) > 1:
                self._network = self._network.module
